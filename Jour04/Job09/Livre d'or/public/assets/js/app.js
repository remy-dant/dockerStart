...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Gestion des actions groupées du livre d'or (cases à cocher + boutons Modifier/Supprimer)
    const bulkForm = document.querySelector('form[action*="bulk_action"]');
    if (bulkForm) {
        const checkboxes = bulkForm.querySelectorAll('input[type="checkbox"][name="selected_comments[]"]');
        ...
        function updateBulkButtons() {
            ...
            // Delete enabled if at least one
            if (deleteBtn) deleteBtn.disabled = count === 0;
            // Edit enabled only if exactly one
        ...
        // Safety: prevent submitting edit when selection count != 1 (in case of manual submit)
        bulkForm.addEventListener('submit', function (e) {
            const action = (document.activeElement && document.activeElement.name === 'action') ? document.activeElement.value : (bulkForm.querySelector('button[name="action"][type="submit"]') || {}).value;
            const checked = Array.from(checkboxes).filter(cb => cb.checked).length;
            if (action === 'edit' && checked !== 1) {
                e.preventDefault();
                alert('Veuillez sélectionner exactement un commentaire à modifier.');
                return false;
            }
            if (action === 'delete' && checked === 0) {
                e.preventDefault();
                alert('Veuillez sélectionner au moins un commentaire à supprimer.');
                return false;
            }
            // For delete, additional confirm
            if (action === 'delete') {
                if (!confirm('Voulez-vous vraiment supprimer les commentaires sélectionnés ?')) {
                    e.preventDefault();
                    return false;
                }
            }
